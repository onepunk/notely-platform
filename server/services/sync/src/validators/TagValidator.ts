 ...
 * Validates tag data before persistence to ensure data integrity
 */
...
export function validate(data: any): ValidationResult {
  ...
  // For non-deleted entities, name and created_at are required
  if (!isDeleted) {
  ...
  if (data.sort_index !== undefined && data.sort_index !== null) {
    if (typeof data.sort_index !== 'number' || !Number.isInteger(data.sort_index)) {
      errors.push('sort_index must be an integer or null');
    }
  }
